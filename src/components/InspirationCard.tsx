...
function extractKeyPhrase(text: string): { main: string; rest: string } {
  ...
  if (sentences.length > 1) {
    ...
    if (shortest.length <= 22) return { main: shortest, rest: '' };
...
const InspirationCard: React.FC<InspirationCardProps> = ({ inspiration, onClick, onUserClick, currentUser }) => {
  ...
  const likesCount = inspiration.stats.likes;
  // 有任何人浇过水就高亮显示图标
  const hasAnyLikes = likesCount > 0;
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        {hasImage ? (
          ...
          >
            <span className="text-5xl font-serif leading-none mb-1 opacity-30 select-none" style={{ color: gradient.text }}>"</span>
            ...
            {rest && (
              <p className="text-center mt-2 opacity-70 line-clamp-2" style={{ color: gradient.text, fontSize: '11px', lineHeight: 1.5 }}>
                {rest}
              </p>
            )}
        ...
        {/* 浇水展示徽章：纯展示，不可点击，有浇水则高亮 */}
        <div className="absolute top-3 right-3">
          <div className={`size-10 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm ${
            hasAnyLikes ? 'bg-primary text-white' : 'bg-white/80 text-slate-300'
          }`}>
            <Droplets size={20} fill={hasAnyLikes ? 'currentColor' : 'none'} />
          </div>
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col gap-2">
        ...
        <div onClick={handleUserClick} className="flex items-center gap-2 mt-1 pt-3 border-t border-primary/5 hover:opacity-70 transition-opacity">
